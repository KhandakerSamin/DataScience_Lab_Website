...
export default function EventsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingItem ? `${API_BASE_URL}/api/events/${editingItem._id}` : `${API_BASE_URL}/api/events`
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to ${editingItem ? "update" : "create"} event`)
      }
      ...
      showToast(`Event ${editingItem ? "updated" : "created"} successfully!`, "success")
    } catch (err) {
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 lg:ml-64 h-full overflow-hidden">
        <div className="lg:hidden bg-white border-b px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold">Events & News</h1>
          <div className="w-10" />
        </div>
        ...
        <div className="h-full overflow-y-auto">
          <div className="px-4 lg:px-6 py-4 max-w-full">
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <div>
                <h1 className="text-xl lg:text-2xl font-bold text-gray-900">Events & News</h1>
                <p className="text-gray-600 text-sm lg:text-base">Manage your events and news</p>
              </div>
              <button
            ...
            {!loading && (
              <div className="overflow-x-auto">
                <DataTable data={data || []} activeTab="events" onEdit={handleEdit} onDelete={handleDeleteClick} />
              </div>
            )}
          </div>
        </div>
