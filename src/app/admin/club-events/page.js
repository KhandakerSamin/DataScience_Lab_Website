...
export default function ClubEventsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingItem ? `${API_BASE_URL}/api/clubEvents/${editingItem._id}` : `${API_BASE_URL}/api/clubEvents`
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to ${editingItem ? "update" : "create"} club event`)
      }
      ...
      showToast(`Club event ${editingItem ? "updated" : "created"} successfully!`, "success")
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/clubEvents/${id}`, {
        method: "DELETE",
      })

      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Failed to delete club event")
      }

      await fetchData()
      showToast("Club event deleted successfully!", "success")
    } catch (err) {
      showToast("Error deleting club event: " + err.message, "error")
    }
  }
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 lg:ml-64 h-full overflow-hidden">
        <div className="lg:hidden bg-white border-b px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold">Club Events</h1>
          <div className="w-10" />
        </div>
          ...
          <div className="px-4 lg:px-6 py-4 max-w-full">
            ...
            {!loading && (
              <div className="overflow-x-auto">
                <DataTable data={data || []} activeTab="clubEvents" onEdit={handleEdit} onDelete={handleDeleteClick} />
              </div>
            )}
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        title="Delete Club Event"
        message="Are you sure you want to delete this club event? This action cannot be undone."
      />
