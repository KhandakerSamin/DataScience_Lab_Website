...
export default function ClubMembersPage() {
  ...
  const fetchClubMembers = async () => {
    try {
      ...
      showToast("Failed to fetch club members: " + err.message, "error")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingItem ? `${API_BASE_URL}/api/clubMembers/${editingItem._id}` : `${API_BASE_URL}/api/clubMembers`
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to ${editingItem ? "update" : "create"} club member`)
      }
      ...
      showToast(`Club member ${editingItem ? "updated" : "created"} successfully!`, "success")
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/clubMembers/${id}`, {
        method: "DELETE",
      })

      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Failed to delete club member")
      }

      await fetchClubMembers()
      showToast("Club member deleted successfully!", "success")
    } catch (err) {
      showToast("Error deleting club member: " + err.message, "error")
    }
  }
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 lg:ml-64 h-full overflow-hidden">
        <div className="lg:hidden bg-white border-b px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold">Club Members</h1>
          <div className="w-10" />
        </div>
        ...
        <div className="h-full overflow-y-auto">
          <div className="px-4 lg:px-6 py-4 max-w-full">
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <div>
                <h1 className="text-xl lg:text-2xl font-bold text-gray-900">Club Members</h1>
                <p className="text-gray-600 text-sm lg:text-base">Manage your club members</p>
              </div>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                {clubMembers.length === 0 && !loading && (
                  <div className="col-span-full text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No Club Members</h3>
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this club member? This action cannot be undone."
      />
