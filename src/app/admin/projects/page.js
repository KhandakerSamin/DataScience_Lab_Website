...
export default function ProjectsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingItem ? `${API_BASE_URL}/api/projects/${editingItem._id}` : `${API_BASE_URL}/api/projects`
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || `Failed to ${editingItem ? "update" : "create"} project`)
      }
          ...
          <div className="px-4 lg:px-6 py-4 max-w-full">
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
              <div>
                ...
                <p className="text-gray-600 text-sm lg:text-base">Manage your projects</p>
              </div>
              <button
            ...
            {!loading && (
              <div className="overflow-x-auto">
                <DataTable data={data || []} activeTab="projects" onEdit={handleEdit} onDelete={handleDeleteClick} />
              </div>
            )}
          </div>
        </div>
