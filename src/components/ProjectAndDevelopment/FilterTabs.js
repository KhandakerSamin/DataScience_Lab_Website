"use client"
import { Filter, FolderOpen, Wrench, Grid3X3 } from "lucide-react"

export default function FilterTabs({ activeFilter, onFilterChange, resultCount }) {
  const filters = [
    { value: "all", label: "All", icon: Grid3X3 },
    { value: "projects", label: "Projects", icon: FolderOpen },
    { value: "developments", label: "Developments", icon: Wrench },
  ]
            ...
            {filters.map(({ value, label, icon: Icon }) => (
              <button
                ...
                className={`flex items-center justify-center gap-2 px-6 py-2 font-medium rounded-md transition-colors duration-200
                  ${activeFilter === value ? "bg-[#09509E] text-white" : "text-[#09509E] hover:bg-[#09509E]/10"}`}
              >
