...
const TeamCard = ({ member, onEdit, onDelete }) => {
  ...
  const socialLinks = [
    { key: "linkedin", url: member.linkedin, label: "LinkedIn" },
    { key: "github", url: member.github, label: "GitHub" },
    { key: "googleScholar", url: member.googleScholar, label: "Google Scholar" },
    { key: "facebook", url: member.facebook, label: "Facebook" },
    { key: "twitter", url: member.twitter, label: "Twitter" },
    { key: "instagram", url: member.instagram, label: "Instagram" },
    { key: "website", url: member.website, label: "Website" },
    { key: "otherLink", url: member.otherLink, label: "Other" },
  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden">
      ...
      <div className="aspect-square w-full bg-gray-200 flex items-center justify-center">
        ...
        {/* Fallback Avatar */}
        <div
          className={`w-20 h-20 bg-gray-300 rounded-full flex items-center justify-center ${member.image ? "hidden" : "flex"}`}
        >
      ...
      <div className="p-4">
        ...
        {member.email && <p className="text-sm text-gray-600 mb-2 truncate">{member.email}</p>}

        {member.bio && <p className="text-sm text-gray-700 mb-3 line-clamp-3">{member.bio}</p>}
        ...
        {socialLinks.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {socialLinks.length > 6 && (
              <span className="text-xs text-gray-500 self-center">+{socialLinks.length - 6} more</span>
            )}
          </div>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex-1 bg-red-600 hover:bg-red-700 text-white text-sm py-2 px-3 rounded-lg transition-colors"
          >
            Delete
          </button>
        </div>
